...
get_sys_stats() {
    ...
    if [[ "$count" == "1" ]] || (( "$count" % 12 == 0 )); then
        ...
        net_gateway=$(ip route | grep default | cut -d ' ' -f 3,5 | head -n 1)
    ...
    #apache
    if [[ "$(pidof apache2 2> /dev/null)" != "" ]]; then 
        apache_status="${COL_LIGHT_GREEN}Active${COL_NC}"
    else
        apache_status="${COL_LIGHT_RED}Offline${COL_NC}"
    fi
    #mysql
    if [[ "$(pidof mariadbd 2> /dev/null)" != "" ]]; then 
...
echo -e "$COL_DARK_GRAY=======================================================$COL_NC"
        ...
        printFunc "  Hostname: " "$sys_name" "$sys_name"
        printFunc "    Uptime: " "$sys_uptime" "$sys_datetime"
        printFunc " Task Load: " "$sys_loadavg" "$sys_info2"
        printFunc " CPU usage: " "$cpu_perc%" "$sys_proc"  #"$cpu_info"
